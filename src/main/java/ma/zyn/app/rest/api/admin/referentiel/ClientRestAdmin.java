...
public class ClientRestAdmin {
    ...
    public ResponseEntity<List<ClientDto>> findAll(){
        ...
        List<ClientDto> dtos  = mapper.toDto(list);
    ...
    public ResponseEntity<ClientDto> save(@RequestBody ClientDto dto){
        if (dto == null) {
            ...
            mapper.init(true);
            Client myT = mapper.toItem(dto);
            Client t = service.create(myT);
            if (t == null) {
				return ResponseEntity.status(HttpStatus.CONFLICT).build();
			} else{
                ClientDto myDto = mapper.toDto(t);
    ...
    public ResponseEntity<ClientDto> update(@RequestBody ClientDto dto){
        ...
        if (dto.getId() == null || service.findById(dto.getId()) == null)
            ...
            ClientDto myDto = mapper.toDto(updated);
    ...
    public ResponseEntity<List<ClientDto>> delete(@RequestBody List<ClientDto> dtos){
        ...
        if (dtos != null && !dtos.isEmpty()) {
            mapper.init(false);
            List<Client> ts = mapper.toItem(dtos);
            service.delete(ts);
            status = HttpStatus.OK;
        }
        res = new ResponseEntity<>(dtos, status);
    ...
    public ResponseEntity<ClientDto> findWithAssociatedLists(@PathVariable Long id) {
        ...
        mapper.init(true);
        ClientDto dto = mapper.toDto(loaded);
   ...
   public ClientRestAdmin(ClientAdminService service, ClientMapper mapper){
